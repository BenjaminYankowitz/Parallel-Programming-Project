...
std::vector<long> neighbor_of(graph mygraph, long node, int world_size)
{
...
std::vector<std::set<long>> generate_RR(graph sub_graph, unsigned long num_sample, int myrank, int world_size)
{
    ...
    long num_node = sub_graph.nodes.size();
    ...
    while (all_finish == 0)
    {
        ...
        while (!frontier.empty())
        {
            ...
            std::vector<long> neighbors = neighbor_of(sub_graph, tuple.node_id, world_size);
